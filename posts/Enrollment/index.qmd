...
Makes one wonder - what's going on with liberal arts schools?

## Liberal arts schools 

If its liberal arts mission is the thing that distinguishes UNCA from other UNC System schools, perhaps it makes sense to extend the comparison to more liberal arts schools?

There were 220 baccalaureate liberal arts schools in the US in 2023, according to their [Carnegie Classification](https://en.wikipedia.org/wiki/Carnegie_Classification_of_Institutions_of_Higher_Education){target=_blank}. A histogram of their Fall 2023 enrollments is shown in @fig-enrollment_histogram below; the dashed vertical line indicates where UNCA falls - at the $92^{\text{nd}}$ percentile.

```{ojs}
//| fig-cap: 2023 enrollment at all 220 liberal arts schools
//| label: fig-enrollment_histogram
{
  const enrollment2023 = await (await 
    FileAttachment('./data/Enrollment2023.csv'))
    .csv({typed:true});
  const UNCAEnrollment = enrollment2023.find(
    o => o.SchoolID == 199111
  ).Enrollment;
  return Plot.plot({
    marks: [
      Plot.rectY(enrollment2023, Plot.binX(
        {y: "count"}, 
        {x: "Enrollment", fill: 'lightgray'}
      )),
...
Let's take a look at how that enrollment has changed over the years.
Note that the graph below allows you to switch back and forth from the total enrollment and relative enrollment views using the radio buttons. Also, the plot below includes only those schools whose enrollment was at least 1000 in 2023. Time series for smaller schools tend to be more volatile, which is particular important when viewing relative enrollment.
...
viewof variable = Inputs.radio(
  ...
  { label: "Show:", value: "Total enrollment" }
);
```
```{ojs}
//| fig-cap: Enrollment and growth at larger liberal arts schools 
make_graph(
  Array.from(grouped_data.get('LiberalArts').values()), 
  variable.toLowerCase().replace(/ /g, "_")
)
```

A key takeaway here is that, even among larger liberal arts schools of all types, UNCA is no outlier; many schools have seen enrollment ups and downs.

A key question now is, what is the appropriate way to deal with enrollment issues in public, higher education?
