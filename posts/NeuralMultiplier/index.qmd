...
The drawing represents almost the most basic type of neural network, sometimes called a *feedforward neural network*. Each circle is called a *node*. The nodes are arranged in *layers* from bottom to top (though, the layers are more conventionally laid out from left to right). The bottom two nodes form the input layer and are assigned values $x_1$ and $x_2$; these are the numbers we want to multiply.
...
As complicated as this might all seem, today's high-quality, machine learning libraries make it amazingly easy in practice. I built the network illustrated here using Python code that you can see in [this Colab notebook](https://colab.research.google.com/drive/19zB1QDbx8ITevYhmv1Speg5RfPNuWUDW?usp=sharing#forceEdit=true&sandboxMode=true){target=_blank}. Here are the critical steps:
...
Note that the model is defined as a sequence of steps. There's an input layer of shape 2 and three "Dense" layers with exactly the size and activation functions we've discussed. The last line specifies how the model should be optimized
...
Note that the edges are labeled with their weights, each of which is $\pm1$. The activation function for the hidden layer is just the square function, $a\to a^2$, and the activation function for the result is the linear function $y\to \frac{1}{4}y$. Finally, the bis for each node is zero.
...
$$
\frac{1}{4}\left((x_1+x_2)^2 - (x_1-x_2)^2\right) = x_1x_2.
$$
